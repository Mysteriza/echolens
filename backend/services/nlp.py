...
class IndoBERTService:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(IndoBERTService, cls).__new__(cls)
    ...
    def __init__(self):
        ...
        if torch.cuda.is_available():
            ...
            if vram > 8 * 1024**3:
                ...
                self.batch_size = 50
            print(f"Hardware: NVIDIA GPU (CUDA) detected. Batch Size: {self.batch_size}")
        elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            self.device_id = "mps"
            self.batch_size = 50
            print(f"Hardware: Apple Silicon (MPS) detected. Batch Size: {self.batch_size}")
        else:
            ...
            if cores >= 8:
                ...
                self.batch_size = 16
            print(f"Hardware: CPU detected ({cores} Cores). Batch Size: {self.batch_size}")
