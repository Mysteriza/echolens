...
class ChatAnswerSchema(BaseModel):
    answer: str = Field(
        description="The natural language answer to the user's question, based on the evidence"
    )
    confidence: str = Field(
        description="High, Medium, or Low depending on how strong the evidence is"
    )
    relevant_aspects: list[str] = Field(
        default_factory=list,
        description="Key aspects mentioned in the answer (e.g. 'battery', 'price')",
    )
    supporting_comment_ids: list[int] = Field(
        default_factory=list,
        description="IDs of the comments from the context that support this answer",
    )
