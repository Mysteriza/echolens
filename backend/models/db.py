...
class Video(Base):
    ...
    analysis_status = Column(String, default="pending")  # pending, collecting, analyzing, completed, failed
...
class Comment(Base):
    ...
    aspects = relationship("CommentAspect", back_populates="comment", cascade="all, delete-orphan")
    embedding = relationship("CommentEmbedding", back_populates="comment", uselist=False, cascade="all, delete-orphan")
...
class VideoReport(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    video_id = Column(Integer, ForeignKey('videos.id', ondelete='CASCADE'), index=True, unique=True)
